...
Auxiliary Space: O(k)
for single array 
and for double array space complexity increases  to O(n*k)
Explanation:
1==========>> n = 0, C(0,0) = 1
...
In statement,
C[j] = C[j] + C[j-1]
Right hand side represents the value coming from previous iteration (A row of Pascal’s triangle depends on previous row). 
